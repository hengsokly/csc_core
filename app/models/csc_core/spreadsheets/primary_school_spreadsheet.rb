...
module CscCore
  module Spreadsheets
    class PrimarySchoolSpreadsheet
      attr_reader :program
      ...
      def process(row)
        ...
        school = ::CscCore::PrimarySchool.find_or_initialize_by(code: row["school_code"].to_s.strip)
      ...
      private
        ...
        def build_school_code(commune)
          num = ::CscCore::PrimarySchool.where(commune_id: commune.id).length + 1
          "#{commune.id}_#{num}"
        end
    end
  end
