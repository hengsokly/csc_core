      ...
      class VotingIndicator
        def self.load(scorecard)
            ...
            scorecard.voting_indicators.create(
              ...
              indicator_activities_attributes: [
                { scorecard_uuid: scorecard.uuid, content: "action1_#{index}", selected: true,
                  type: "CscCore::SuggestedIndicatorActivity" },
          ...
          customs.take(1).each do |indi|
            scorecard.voting_indicators.create(indicator_uuid: indi["uuid"], indicatorable_id: indi["id"],
                                               indicatorable_type: "CscCore::Indicators::CustomIndicator")
          end
        end
      end
    end
